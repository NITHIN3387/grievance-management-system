...
const page = () => {
  ...
  useEffect(() => {
    ...
    const auth = async () => {
      await authUser()
        ...
        .catch((err) => {
          console.log("fail to fetch admin details\n", err);
        });
    };
  ...
  const filter = (data, action) => {
    if (!data.description.toLowerCase().includes(search)) return false;

    if (!poor && !pending && !onProgress && !solved) return true;

    return poor && action.status == "poor"
      ? true
      : pending && action.status == "pending"
      ? true
      : onProgress && action.status == "on progress"
      ? true
      : solved && action.status == "solved"
      ? true
      : false;
  };
        ...
        <div className="grid gap-5">
          ...
          <div className="flex sm:gap-5 gap-3">
            <div
              ...
              onClick={handlePoorFilter}
              ref={filterBtnPoorRef}
            >
              Poor
            </div>
            <div
